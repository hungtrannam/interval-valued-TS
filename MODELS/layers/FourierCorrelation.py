...
class FourierCrossAttention(nn.Module):
    ...
    def forward(self, q, k, v, mask):
        ...
        if self.activation == 'tanh':
            ...
            mag = torch.abs(xqk_ft)
            softmax_mag = torch.softmax(mag, dim=-1)
            xqk_ft = torch.complex(softmax_mag, torch.zeros_like(softmax_mag))
        elif self.activation == 'sigmoid':
            xqk_ft = torch.complex(xqk_ft.real.sigmoid(), xqk_ft.imag.sigmoid())
        elif self.activation == 'sine':
            xqk_ft = torch.complex(torch.sin(xqk_ft.real), torch.sin(xqk_ft.imag))
        elif self.activation == 'cosine':
            xqk_ft = torch.complex(torch.cos(xqk_ft.real), torch.cos(xqk_ft.imag))
        elif self.activation == 'relu':
            xqk_ft = torch.complex(torch.relu(xqk_ft.real), torch.relu(xqk_ft.imag))
        elif self.activation == 'silu':
            xqk_ft = torch.complex(torch.nn.functional.silu(xqk_ft.real), torch.nn.functional.silu(xqk_ft.imag))

        elif self.activation == 'gelu':
            xqk_ft = torch.complex(torch.nn.functional.gelu(xqk_ft.real), torch.nn.functional.gelu(xqk_ft.imag))

        else:
            raise Exception(f"Activation function '{self.activation}' is not implemented.")
